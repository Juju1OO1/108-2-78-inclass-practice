...
```{r ans12}
item1 <- map(cpiByIncome,~.x[['Item']])
items <- factor(unlist(item1))
...
```{r ans14}
items -> subcategory
levels(subcategory) -> currentLevels1
str_extract(currentLevels1,"(?<=#)[:graph:]+(?=\\()") -> newLevels1
newLevels1 -> levels(subcategory)
...
```{r ans15}
factor(subcategory, levels="一.食物類","二.衣著類",",")

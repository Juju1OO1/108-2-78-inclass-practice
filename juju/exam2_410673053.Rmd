...
```{r ans13}
regex_hs1st <- "[臺台]?北(?!市立第)一女(?!子高級)中?(?!學)"
...
```{r ans21}
data228[["全宗名編號"]] <- str_extract_all(data228[['檔號']],"[:alnum:]{11}(?=/)")
data228[["年"]] <- str_extract_all(data228[['檔號']],"(?<=/00)[:alnum:]{2}(?=/)")
...
```{r ans22}
pick_numbers <- data228[['附註']]=="表冊名稱:臺灣省二二八事變正法及死亡人犯名冊"
numbers <- data228[['檔號']][pick_numbers]
pick_victims <- data228[['檔號']]%in%numbers
victims <- data228[['個人名稱']][pick_victims]
...
```{r ans23}
victims <- factor(victims)
documentCount_eachVictim <- table(victims,ordered=FALSE)
...
```{r ans32}
pick_year <- courseStructure[['學年']]=="103"
pick_grade <- stringr::str_detect(courseStructure[['應修系級']],"經濟系3")
names <- courseStructure[['科目名稱']][pick_year&pick_grade]
econ101junior <- levels(factor(names))

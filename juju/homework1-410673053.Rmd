...
```{r ans21c}
*
#要在右下角的package尋找lubridate 並且打勾把這個package加到library裡面 這樣程式執行的時候才找的到
```
...
```{r ans44}
numbers <- as.character(fiveReceipts)
sum <- as.integer(numbers)
sum_fiveReceipts <- sum(sum)
sum_fiveReceipts
```

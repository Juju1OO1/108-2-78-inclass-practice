...
```{r ans32}
ATK <- Pikachu[["baseStats"]][["atk"]]
DEF <- Pikachu[["baseStats"]][["def"]]
HP <- Pikachu[["baseStats"]][["hp"]]
Pikachu_stats1 <- c(ATK,DEF,HP)
Pikachu_stats <- as.numeric(Pikachu_stats1)
...
```{r ans33}
pudding <- Gamemaster[["pokemon"]][[46]]
pudding[["speciesName"]] <- "Jigglypuff"
Jigglypuff <- pudding
...
```{r ans34}
#1.取出皮卡丘的baseStats
Pikachu1 <- Gamemaster[["pokemon"]][[27]][["baseStats"]]
#2.取出胖丁的Moves
Jigglypuff1 <- Jigglypuff[["fastMoves"]]
Jigglypuff2 <- Jigglypuff[["chargedMoves"]]
Jigglypuff3 <- Jigglypuff[["legacyMoves"]]
#3.代入數據
Gamemaster[["pokemon"]][[731]] <- c(list(`dex`=as.integer(778),`speciesName`="Mimikyu",`speciesId`="mimikyu",`baseStats`=Pikachu1,`types`=c("ghost","fairy"),`fastMoves`=Jigglypuff1,`chargedMoves`=Jigglypuff2,`legacyMoves`=Jigglypuff3,`defaultIVs`=list(`cp1500`=as.integer(c(40,15,15,15)),`cp2500`=as.integer(c(40,15,15,15)))))

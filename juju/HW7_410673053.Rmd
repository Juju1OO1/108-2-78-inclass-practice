...
lastnumber <- as.numeric(stringr::str_sub(Mask[[1]][[1]],10))

`1st_odd_or_even` <- if(lastnumber%%2==1){
  print(1)
} else {
  print(0)
}
...
```{r ans22}
library(stringr)
odd_or_even <- function(n){if(as.numeric(str_sub(n,10))%%2==1){print(1)} else {print(0)}}
...
n <- Mask[['身分證字號']][[1]]
function(n){if(wday(today("Asia/Taipei"))%in%chart[[odd_or_even(n)+1]]){`1st_next_date`=today("Asia/Taipei")} else {`1st_next_date`=today("Asia/Taipei")+dhours(24)}}
...
```{r ans24}
today <- paste0(today("Asia/Taipei"),",",weekdays.Date(today("Asia/Taipei")))
otherday <- paste0(today("Asia/Taipei")+dhours(24),",",weekdays.Date(today("Asia/Taipei")+dhours(24)))
next_date <- function(n){if(wday(today("Asia/Taipei"))%in%chart[[odd_or_even(n)+1]]){print(today)} else {print(otherday)}}
...
factor(c(entranceYear, major))

pick_major <- str_extract_all(courseStructure[['應修系級']],"[一-\u9fff]+")%in%str_extract_all(major,"[一-\u9fff]+")

pick_grade1 <- str_detect(courseStructure[['應修系級']],"1")
pick_grade2 <- str_detect(courseStructure[['應修系級']],"2")
pick_grade3 <- str_detect(courseStructure[['應修系級']],"3")
pick_grade4 <- str_detect(courseStructure[['應修系級']],"4")

return(list(
year1 <- levels(factor(courseStructure[['科目名稱']][pick_major&pick_grade1])),
year2 <- levels(factor(courseStructure[['科目名稱']][pick_major&pick_grade2])),
year3 <- levels(factor(courseStructure[['科目名稱']][pick_major&pick_grade3])),
year4 <- levels(factor(courseStructure[['科目名稱']][pick_major&pick_grade4]))))}



get_courses(100,"經濟系2")
# get_courses
```
